...
export default async function BlogPost({ params, searchParams }: Props) {
  ...
  const postCardIds =
    processedBody
      ?.filter((b): b is Extract<BodyBlock, { fieldId: "postCard" }> => b.fieldId === "postCard")
      .map((b) => b.post?.id)
      .filter((v): v is string => typeof v === "string" && v.length > 0) ?? [];

  const postCards = await getBlogCardsByIds(postCardIds);
            ...
            {processedBody && processedBody.length > 0 ? (
              processedBody.map((block, index) => {
                switch (block.fieldId) {
                  ...
                  case "linkButton":
                    return <RenderLinkButton key={index} url={block.url} label={block.label} color={block.color} />;
                  case "pointBox":
                    return <RenderPointBox key={index} content={block.content} type={block.type} />;
                  case "postCard": {
                    const refId = block.post?.id;
                    if (!refId) return null;

                    const linked = postCardMap.get(refId);
                    if (!linked) return null;

                    return <PostLinkCard key={index} post={linked} />;
                  }
                  default:
                    return null;
                }
              ...
              <div className="prose prose-lg max-w-none text-gray-800">{parse(processedContent)}</div>
            )}
          </div>
